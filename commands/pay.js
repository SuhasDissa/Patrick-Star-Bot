...
module.exports.run = async (bot, message, args) => {
  ...
  if (!money[message.author.id])
    return message.reply("Oh poor friend. you dont have money!");
  if (!Number.isInteger(parseInt(args[1]))) return;

  if (parseInt(args[1]) > money[message.author.id].money)
    return message.reply("You dont have that much money");
  ...
  return message.channel.send(
    `${message.author.username} payed ${args[1]} coins to ${
